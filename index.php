...
$labels = [
    'ru' => ['title' => 'Регистрация', 'name'=> 'Имя', 'surname' => 'Фамилия', 'email' => 'Почта', 'age' => 'Возраст', 'password' => 'Пароль', 'gender' => 'Пол', 'languages' => 'Языки'],
    'ua' => ['title' => 'Реєстрація', 'name'=> "Им'я", 'surname' => 'Прізвище', 'email' => 'Пошта', 'age' => 'Вік', 'password' => 'Пароль', 'gender' => 'Стать', 'languages' => 'Мови'],
    'en' => ['title' => 'Registration', 'name'=> 'Name', 'surname' => 'Surname', 'email' => 'Email', 'age' => 'Age', 'password' => 'Password', 'gender' => 'Gender', 'languages' => 'Languages']
];
...
if (!empty($_POST)) {
    ...
    if (empty($_POST['languages'])) {
        $error['languages'] = 'Выберите хотя-бы один язык';
    }
...
<div class="container">
    ...
    <form method="post" action="index.php">
        ...
        <div class="form-group">
            <label for="exampleFormControlSelect1"><?=$translation['languages']?></label>
            <select multiple class="form-control" id="exampleFormControlSelect1" name="languages[]">
                <option <?=$_POST['languages'] == 'English' ? 'selected': ''?>>English</option>
                <option <?=$_POST['languages'] == 'Russian' ? 'selected': ''?>>Russian</option>
                <option <?=$_POST['languages'] == 'Ukrainian' ? 'selected': ''?>>Ukrainian</option>
                <option <?=$_POST['languages'] == 'German' ? 'selected': ''?>>German</option>
                <option <?=$_POST['languages'] == 'Polish' ? 'selected': ''?>>Polish</option>
                <option <?=$_POST['languages'] == 'French' ? 'selected': ''?>>French</option>
                <option <?=$_POST['languages'] == 'Japanese' ? 'selected': ''?>>Japanese</option>
                <option <?=$_POST['languages'] == 'Chinese' ? 'selected': ''?>>Chinese</option>
            </select>
            <?php if(!empty($error['languages'])) : ?>
                <small style="color:red" id="passwordHelpBlock" class="from-text">
                    <?= $error['languages'] ?>
                </small>
            <?php endif; ?>
